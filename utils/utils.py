...
def init_model(device="cuda", sd_version="1.5", model_key=None, control_type="none", weight_dtype="fp16"):
    ...
    if model_key is None:
        if sd_version == '2.1':
            ...
            model_key = "/mnt/fspace/hproject/model/stable-diffusion-v1-5/"
...
def save_video_with_ffmpeg(frames, output_path, fps=30):
    ...
    for i, frame in enumerate(frames):
        ...
        T.ToPILImage()(frame).save(frame_path)

    video_path = os.path.join(output_path, "output.mp4")
    ffmpeg_cmd = [
        "ffmpeg", "-y", "-framerate", str(fps), "-i", f"{temp_dir}/%04d.png",
        "-c:v", "libx264", "-crf", "0", "-preset", "slow", video_path
    ]
